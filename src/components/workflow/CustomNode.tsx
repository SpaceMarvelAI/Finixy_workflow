...
const isDescriptionMeaningful = (
  ...
  if (cleanDesc.replace(/\s+/g, "") === cleanName.replace(/\s+/g, ""))
    return false;

  // Filter out if name fully contains description or vice versa
  if (cleanName.includes(cleanDesc) || cleanDesc.includes(cleanName))
    return false;
  ...
  const intersection = new Set(
    [...descWordSet].filter((word) => nameWordSet.has(word)),
  );
  const overlapRatio =
    intersection.size / Math.min(descWordSet.size, nameWordSet.size);
  ...
  if (descWords.length > 0 && partialMatches / descWords.length > 0.6)
    return false;
  ...
  if (descNoSpaces.length >= 3 && nameNoSpaces.includes(descNoSpaces))
    return false;
  if (nameNoSpaces.length >= 3 && descNoSpaces.includes(nameNoSpaces))
    return false;
...
const CustomNodeComponent: React.FC<CustomNodeProps> = ({ data, id }) => {
  ...
  const description = isDescriptionMeaningful(rawDescription, name)
    ? rawDescription
    : "";
  ...
  const isFirstNode = safeData.isFirstNode === true;
  ...
  return (
    ...
    >
                                      ...
                                      : colorClass.includes("emerald")
                                        ...
                                        : colorClass.includes("sky")
                                          ...
                                          : colorClass.includes("violet")
                                            ? "#8b5cf6"
                                            : colorClass.includes("fuchsia")
                                              ? "#d946ef"
                                              : colorClass.includes("slate")
                                                ? "#64748b"
                                                : "#6b7280"
          } 20%, rgba(0, 0, 0, 0.3) 100%)`,
        }}
      >
        <div className="p-1.5 bg-white/20 rounded-lg backdrop-blur-sm">
          {icon}
        </div>
        <span className="font-semibold text-sm capitalize tracking-wide flex-1">
          {name}
        </span>

        {/* 3-dot menu — only on first node of workflow */}
        {isFirstNode && (
          <div
            ref={menuRef}
            className="relative"
      ...
      {/* Dropdown — rendered outside header, anchored to node right edge */}
      {isFirstNode && menuOpen && (
        <div
          ref={null}
          className="absolute top-1/2 -translate-y-1/2 z-50 bg-theme-secondary border border-theme-primary rounded shadow-xl min-w-[160px] overflow-hidden"
        ...
        >
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 text-xs text-emerald-500 hover:bg-theme-tertiary transition-colors"
          >
          ...
          <button
            onClick={() => {
              setMenuOpen(false);
              data.onDeleteWorkflow?.();
            }}
            className="w-full flex items-center gap-2 px-3 py-2 text-xs text-red-400 hover:bg-theme-tertiary transition-colors"
          >
