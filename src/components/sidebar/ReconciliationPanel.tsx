...
export const ReconciliationPanel: React.FC<ReconciliationPanelProps> = ({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    // Stubbed — replace with real API call when backend is ready
  ...
  const companies = Array.from(
    new Set(items.map((s) => s.company_name).filter(Boolean)),
  ).sort() as string[];
          ...
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-theme-tertiary rounded-lg flex items-center justify-center shadow-lg">
                <GitCompare className="w-4 h-4 text-theme-secondary" />
              </div>
              <div>
            ...
            <div className="flex flex-col items-center justify-center h-40 gap-3 text-theme-tertiary">
              ...
              <p className="text-sm">
                {search
                  ...
                  : "No reconciliation files uploaded yet"}
              </p>
              <button
                onClick={() => fileInputRef.current?.click()}
            ...
            <table className="w-full text-xs">
              <thead className="sticky top-0 theme-table-head border-b z-10">
                <tr>
                  ...
                  <th className="px-3 py-3 text-center text-[10px] font-bold uppercase tracking-widest w-[5%]">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-theme-primary">
                {filtered.map((s, idx) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <span
                        className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border ${getStatusColor(s.status)}`}
                      >
                        {s.status || "pending"}
                      </span>
                    </td>
